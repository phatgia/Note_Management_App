...
export default function SharedNote({ notes, categories }: any) {
    ...
    return (
        <NoteLayout title="Ghi chú được chia sẻ">
            ...
            <div className="sticky top-0 bg-card flex items-center justify-between border-b border-gray-200 dark:border-gray-700 p-6 z-10 transition-colors">
                <h1 className="text-2xl font-bold text-foreground">Ghi chú được chia sẻ</h1>
                
                <div className="flex items-center gap-4">
                    <button onClick={gridView} className={`cursor-pointer border border-gray-200 dark:border-gray-700 rounded-md px-4 py-2 transition-colors ${viewMode === 'grid' ? 'bg-orange-500 text-white' : 'bg-card text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                    ...
                    <button onClick={listview} className={`cursor-pointer border border-gray-200 dark:border-gray-700 rounded-md px-4 py-2 transition-colors ${viewMode === 'list' ? 'bg-orange-500 text-white' : 'bg-card text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                    ...
                    <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6 gap-6 m-3" : "flex flex-col gap-4 m-3"}>
                        {editorNotes.map((note: any) => (
                            <Link href={`/note-detail/${note.id}`} key={note.id} className={`${note.bg_color || 'bg-white'} dark:bg-gray-800/90 border border-orange-200 dark:border-gray-700 rounded-2xl p-5 flex flex-col min-h-[16rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer`}>
                                ...
                                <h3 className="font-bold text-card-foreground text-lg mb-2 line-clamp-1" title={note.title}>{note.title}</h3>
                                ...
                                <div className="border-t border-gray-100 dark:border-gray-700/50 mt-auto pt-4 flex justify-between items-center">
