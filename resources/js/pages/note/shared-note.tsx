    ...
    return (
        <NoteLayout title="Ghi chú được chia sẻ">
            <div className="w-full bg-[#F8F9FA] dark:bg-background min-h-screen pb-12 overflow-y-auto">
                <div className="sticky top-0 bg-white dark:bg-card flex items-center justify-between border-b border-gray-200 dark:border-gray-700 p-6 z-10 transition-colors shadow-sm">
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Ghi chú được chia sẻ</h1>
                    <div className="flex items-center gap-4">
                        <button onClick={() => updateViewMode('grid')} className={`cursor-pointer border border-gray-200 dark:border-gray-700 rounded-md px-4 py-2 transition-colors ${viewMode === 'grid' ? 'bg-orange-500 border-orange-500 text-white shadow-sm' : 'bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>
                        ...
                        <button onClick={() => updateViewMode('list')} className={`cursor-pointer border border-gray-200 dark:border-gray-700 rounded-md px-4 py-2 transition-colors ${viewMode === 'list' ? 'bg-orange-500 border-orange-500 text-white shadow-sm' : 'bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>
                ...
                <div className="p-6">
                    <div className="mb-10">
                        <div className="flex items-center gap-3 mb-6">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-orange-500"><path d="M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.158 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-8.4 8.4a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32l8.4-8.4Z" /><path d="M5.25 5.25a3 3 0 0 0-3 3v10.5a3 3 0 0 0 3 3h10.5a3 3 0 0 0 3-3V13.5a.75.75 0 0 0-1.5 0v5.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5V8.25a1.5 1.5 0 0 1 1.5-1.5h5.25a.75.75 0 0 0 0-1.5H5.25Z" /></svg>
                            <p className="font-semibold text-gray-700 dark:text-gray-300">Có thể chỉnh sửa</p>
                            <div className="flex-1 h-[1px] bg-gray-200 dark:bg-gray-700/60"></div>
                        </div>
                        {editorNotes.length === 0 ? (
                            <div className="col-span-full flex flex-col items-center justify-center py-10 bg-white/50 dark:bg-card/50 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 text-gray-400">
                            ...
                            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6 gap-6" : "flex flex-col gap-4"}>
                                {editorNotes.map((note: any, index: number) => (
                                    <Link href={`/note-detail/${note.id}`} key={`${note.id}-${index}`} className={`${note.bg_color || 'bg-white'} dark:bg-gray-800/90 border border-orange-200 dark:border-gray-700 rounded-2xl p-5 flex flex-col min-h-[16rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer`}>
                                        ...
                                        {note.categories && note.categories.length > 0 && (
                                            <div className="flex flex-wrap gap-2 mb-4 mt-2">
                                                {note.categories.map((cat: any, catIdx: number) => (
                                                    <span key={`${cat.id}-${catIdx}`} className={`text-xs font-bold px-2.5 py-1 rounded-full border ${cat.color ? cat.color : 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                                                        {cat.name || cat.Name}
                                                    </span>
                                                ))}
                                            </div>
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <p className="font-semibold text-gray-700 dark:text-gray-300">Chỉ xem</p>
                            <div className="flex-1 h-[1px] bg-gray-200 dark:bg-gray-700/60"></div>
                        </div>
                        {viewerNotes.length === 0 ? (
                            <div className="col-span-full flex flex-col items-center justify-center py-10 bg-white/50 dark:bg-card/50 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 text-gray-400">
                                <svg className="w-12 h-12 mb-3 text-gray-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /></svg>
                                <p className="text-sm">Chưa có ghi chú nào được chia sẻ quyền xem.</p>
                            </div>
                        ) : (
                            <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6 gap-6" : "flex flex-col gap-4"}>
                                {viewerNotes.map((note: any, index: number) => (
                                    <Link href={`/note-detail/${note.id}`} key={`${note.id}-${index}`} className={`${note.bg_color || 'bg-white'} dark:bg-gray-800/90 border border-blue-200 dark:border-gray-700 rounded-2xl p-5 flex flex-col min-h-[16rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer`}>
                                        <div className="flex items-center gap-2 mb-4">
                                            <div className="bg-blue-50 dark:bg-gray-900 border border-transparent dark:border-blue-500/30 text-blue-500 p-1.5 rounded-lg">
